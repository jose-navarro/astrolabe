/** \file r_matrix_reader.cpp
  \brief  Implementation file for r_matrix_reader.hpp.
...
{
  {
    ...
    if (data_channel_is_socket_)
    {
      ...
      try
      {
        socket_reader_ = new r_matrix_socket_reader();
      }
      catch (...)
      {
        // Unable to open.
    ...
    else
    {
      ...
      if (is_binary_file)
      {
        ...
        if (is_backwards_mode_set_)
        {
          ...
          file_reader_ = new r_matrix_file_reader_bin_backward();
        }
        else
        {
          // Forward, binary file reader.
          file_reader_ = new r_matrix_file_reader_bin_forward();
        }
      }
      else
      {
        ...
        if (is_backwards_mode_set_)
        {
          ...
          file_reader_ = new r_matrix_file_reader_txt_backward();
        }
        else
        {
          // Forward, text file reader.
          file_reader_ = new r_matrix_file_reader_txt_forward();
        }
      }
...
{
  {
    if (data_channel_is_socket_)
    {
      ...
      return socket_reader_->read_r_data(n_correlation_values_found,
                                         the_correlation_values);
    }
    else
    {
      // Files

      return file_reader_->read_r_data(n_correlation_values_found,
                                       the_correlation_values);
    }
  }
}
