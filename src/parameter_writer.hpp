...
/**
   \brief  Class able to write paramter data to all kinds of
   ASTROLABE supported data channels.
   ...
   THEREFORE, THIS IS THE CLASS TO USE WHEN WRITING ASTROLABE PARAMETER DATA. All other
...
class parameter_writer
{
  public:

    /**
     * \brief Closes the parameter writer, disabling any other
     ...
     *
     * After an parameter writer has been instantiated, the parameters defining the
  ...
  protected:
    ...
    /// \brief Socket writer in charge to write parameter data
    /// using a socket conection.

    parameter_socket_writer*        socket_writer_;
};

#endif // PARAMETER_WRITER_HPP
