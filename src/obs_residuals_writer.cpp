/** \file obs_residuals_writer.cpp
  \brief  Implementation file for obs_residuals_writer.hpp.
  \ingroup ASTROLABE_data_IO
*/

#include "obs_residuals_writer.hpp"
...
{
  {
    ...
    //
    // We work with files only. In all cases we must write the astrolabe
    // header file including the metadata defining the data channel.
    //
    ...
    if (file_mode_is_binary_)
    {
      ...
      file_writer_ = new obs_residuals_file_writer_bin();
    ...
    else
    {
      ...
      file_writer_ = new obs_residuals_file_writer_txt();
...
{
  {
    ...
    //
    // We work with files only, either binary or text.
    //
