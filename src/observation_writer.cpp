...
{
  {
    ...
    if (data_channel_is_socket_)
    {
      ...
      socket_writer_ = new observation_socket_writer();
    ...
    else
    {
      ...
      if (file_mode_is_binary_)
      {
        ...
        file_writer_ = new observation_file_writer_bin();
      ...
      else
      {
        ...
        file_writer_ = new observation_file_writer_txt();
...
{
  {
    ...
    if (data_channel_is_socket_)
    {
      ...
      status = socket_writer_->write_o(active, identifier, time,
                                               n_parameter_iids,   the_parameter_iids,
    ...
    else
    {
      ...
      // Files, either binary or text.
      //
      // We'll use the polymorphic writer to write the o-record.
      //
      // We must check if this new o-record belongs to a brand new
      // epoch. If so, we're facing an invalid situation and must
      // report an error. Therefore, we must decide if this new
      // record belongs to an already existing epoch or opens a
      // new one...
      //
      ...
      {
        //
        // The epoch changes just with an o-record. This is an error! The first
        // record in any epoch must be an l one. So we report the error and
        // quit.
        //
      ...
      // Finally, write the o-record! Use the polymorphic writer to do it.

      status = file_writer_->write_o(active, identifier, time,
                                              n_parameter_iids,   the_parameter_iids,
                                              n_observation_iids, the_observation_iids,
                                              n_instrument_iids,  the_instrument_iids);
      return status;
