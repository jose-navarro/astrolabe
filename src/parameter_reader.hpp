/** \file parameter_reader.hpp
  \brief  Class able to read parameter data from all kinds of
...
#include "parameter_file_reader_txt_forward.hpp"
...
/**
   ...
   This class is able to read parameter data from all the different types of
   ...
   text. Beware that parameter files can only be read in forward direction
   (unlike observation files that may be read also backwards).

   THIS IS THE CLASS TO USE WHEN READING ASTROLABE PARAMETER DATA. All other
   ...
   - Instantiate the parameter_reader object (either automatically or using explicitly
     its constructor).
   ...
   - Open the reader using method open(). Note that this reader, will directly
     read the underlying data channel (parameter data).
   ...
   This reader, therefore, use XTL to decode the information sent by the parameter
...
class parameter_reader
{
  public:
     ...
     *
     * All records in an parameter channel are ordered by time. All those sharing
     * the same time tag constitute an epoch.
     *
     * When reading successive records in an parameter channel, soon or later
    ...
    /**
     * @brief Check if the data in the parameter channel has been completely read.
     *
     * @return True if all data in the parameter channel has already been read,
     *        false otherwise.
     *
     * Use this method to check if the contents of the parameter channel has
     ...
     *         - 2: I/O error while reading (might happen if the parameter
     ...
     *
     * If an identifier is found in the input parameter channel whose length is greater than
     ...
     *         - 0: Successful completion.
     *         - 1: End of file (no more data).
     *         - 2: I/O error while reading (might happen if the parameter channel
     *              has not been opened).
     *         - 3: Invalid call sequence (see description below about
     *              the proper call sequence).
     ...
     * be called right after the parameter_reader has been instantiated
  ...
  protected:
     ...
     *
     * Warnings may be ignored and the process of reading the parameters
     ...
     *
     * On the contrary, errors imply that the parameter file may not
     * be read.
     *
  ...
  protected:
    ...
    /// \brief Flag. When true, indicates that the underlying parameter
    /// channel is open.
    ...
    parameter_socket_reader*        socket_reader_;
