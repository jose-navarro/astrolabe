...
(int argc, char** argv)
{
  ...
  // The name of the output file. Port used
  // by the socket reader.
  string                         outfile_name;
  int                            port;

  // The socket reader and text writer.

  observation_socket_reader*     socket_reader;
  ...
  // the name of the program): one for the port used to read via
  // sockets and one more for the output text file name.
  //

  if (argc < 3)
  {
    cout << "usage: test_socket_to_txt port_number output_txt_file_name" << endl;
    return 1;
  }
  ...
  // Instantiate the new text file writer.
  ...
  // Try to open the text file writer.
  ...
  if (io_status != 0)
  {
    ...
    cout << "[ERROR] open (text writer): " << io_status << endl;
    return 1;
  }

  // Instantiate the new socket reader.

  socket_reader = new observation_socket_reader();

  //
  // Try to open socket reader. We'll work in server mode,
  // so we need no host name.
  //

  io_status = socket_reader->open(NULL, port, true);
  if (io_status != 0)
  {
    delete identifier;
    delete the_tags;
    ...
    cout << "[ERROR] open (socket reader): " << io_status << endl;
    return 1;
  }

  //
  // Try to read the data received through the socket reader and, simultaneously,
  // write it using the text file writer.
  //
  ...
  while (true)
  {
    ...
    io_status = socket_reader->read_type(type);
    if (io_status != 0)
    {
      //
    ...
    io_status = socket_reader->read_active_flag(active);
    if (io_status != 0)
    {
    ...
    io_status = socket_reader->read_identifier(identifier, identifier_width);
    if (io_status != 0)
    {
    ...
    if (type == 'l')
    {
      io_status = socket_reader->read_instance_id(instance_identifier);
      if (io_status != 0)
      {
    ...
    io_status = socket_reader->read_time(time);
    if (io_status != 0)
    {
    ...
    if (type == 'l')
    {
      ...
      io_status = socket_reader->read_l_data(n_tags,              the_tags,
    ...
    else // (type == 'o')
    {
      ...
      io_status = socket_reader->read_o_data(n_par_iids, the_par_iids,
  ...
  // Try to close the socket reader.

  i = 0;

  io_status = socket_reader->close();
  if (io_status != 0)
  {
    cout << "[ERROR] close (socket reader): " << io_status << endl;
    i = 1;
  }

  // Try to close the text file writer.

  io_status = text_writer->close();
  if (io_status != 0)
  {
    cout << "[ERROR] close (text writer): " << io_status << endl;
    i = 1;
  }
