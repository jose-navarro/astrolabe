...
(int argc, char** argv)
{
  ...
  observation_file_reader_txt_backward* reverse_text_reader;
  ...
  // another one for the output text file.
  //

  if (argc < 3)
  {
    cout << "usage: test_txt_to_reverse_txt input_txt_file_name output_txt_file_name" << endl;
    return 1;
  }
  ...
  // Instantiate the new text backwards reader.

  reverse_text_reader = new observation_file_reader_txt_backward();
  ...
  reverse_text_reader->set_buffer_size(5 * 1024); // Just 5 Kbytes.

  // Try to open the file.

  io_status = reverse_text_reader->open(infile_name.c_str());
  if (io_status != 0)
  {
    delete identifier;
    delete the_tags;
    ...
    cout << "[ERROR] open (input text): " << io_status << endl;
    return 1;
  }
  ...
  if (io_status != 0)
  {
    ...
    cout << "[ERROR] open (output txt): " << io_status << endl;
    return 1;
  }
  ...
  while (true)
  {
    ...
    io_status = reverse_text_reader->read_type(type);
    if (io_status != 0)
    {
      //
    ...
    io_status = reverse_text_reader->read_active_flag(active);
    if (io_status != 0)
    {
    ...
    io_status = reverse_text_reader->read_identifier(identifier, identifier_width);
    if (io_status != 0)
    {
    ...
    if (type == 'l')
    {
      io_status = reverse_text_reader->read_instance_id(instance_identifier);
      if (io_status != 0)
      {
    ...
    io_status = reverse_text_reader->read_time(time);
    if (io_status != 0)
    {
    ...
    if (type == 'l')
    {
      ...
      io_status = reverse_text_reader->read_l_data(n_tags,              the_tags,
    ...
    else // (type == 'o')
    {
      ...
      io_status = reverse_text_reader->read_o_data(n_par_iids, the_par_iids,
  ...
  io_status = reverse_text_reader->close();
  if (io_status != 0)
  {
    cout << "[ERROR] close (input txt): " << io_status << endl;
    i = 1;
  }
  ...
  if (io_status != 0)
  {
    cout << "[ERROR] close (output txt): " << io_status << endl;
    i = 1;
  }

  delete reverse_text_reader;
  delete text_writer;
  delete identifier;
  delete the_tags;
  delete the_expectations;
