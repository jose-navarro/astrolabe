...
(int argc, char** argv)
{
  ...
  // The name of the intput and output filef.

  string                               infile_name;
  string                               outfile_name;

  // The binary reader (forward) and text writer objects.

  observation_file_reader_bin_forward* binary_reader;
  observation_file_writer_txt*         text_writer;
  ...
  // the name of the program): one for the input text file and
  // another one for the output binary file.
  //

  if (argc < 3)
  {
    cout << "usage: test_bin_to_txt input_bin_file_name output_txt_file_name" << endl;
    return 1;
  }
  ...
  // Instantiate the new binary forward reader.

  binary_reader = new observation_file_reader_bin_forward();
  ...
  binary_reader->set_buffer_size(5 * 1024); // Just 5 Kbytes.

  // Try to open the file.

  io_status = binary_reader->open(infile_name.c_str());
  if (io_status != 0)
  {
    delete identifier;
    delete the_tags;
    ...
    cout << "[ERROR] open (binary): " << io_status << endl;
    return 1;
  }

  // Instantiate the new text writer.

  text_writer = new observation_file_writer_txt();
  ...
  text_writer->set_buffer_size(5 * 1024); // Just 5 Kbytes.

  // Try to open the file.

  io_status = text_writer->open(outfile_name.c_str());
  if (io_status != 0)
  {
    delete identifier;
    delete the_tags;
  ...
  while (true)
  {
    ...
    io_status = binary_reader->read_type(type);
    if (io_status != 0)
    {
      //
    ...
    io_status = binary_reader->read_active_flag(active);
    if (io_status != 0)
    {
    ...
    io_status = binary_reader->read_identifier(identifier, identifier_width);
    if (io_status != 0)
    {
    ...
    if (type == 'l')
    {
      io_status = binary_reader->read_instance_id(instance_identifier);
      if (io_status != 0)
      {
    ...
    io_status = binary_reader->read_time(time);
    if (io_status != 0)
    {
    ...
    if (type == 'l')
    {
      ...
      io_status = binary_reader->read_l_data(n_tags,              the_tags,
      ...
      // OK, let's write the l-record to the text file.

      io_status = text_writer->write_l(active, identifier, instance_identifier, time,
                                       n_tags, the_tags,
                                       n_expectations, the_expectations,
                                       n_covariance_values, the_covariance_values);
    ...
    else // (type == 'o')
    {
      ...
      io_status = binary_reader->read_o_data(n_par_iids, the_par_iids,
      ...
      // OK, let's write the o-record to the text file.

      io_status = text_writer->write_o(active, identifier, time,
  ...
  io_status = binary_reader->close();
  if (io_status != 0)
  {
    cout << "[ERROR] close (binary): " << io_status << endl;
    i = 1;
  }

  // Try to close the output file.

  io_status = text_writer->close();
  if (io_status != 0)
  {
    cout << "[ERROR] close (txt): " << io_status << endl;
    i = 1;
  }
