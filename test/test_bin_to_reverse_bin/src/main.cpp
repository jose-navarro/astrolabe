...
(int argc, char** argv)
{
  ...
  // The name of the intput and output files.

  string                                infile_name;
  string                                outfile_name;

  // The binary reader (backwards) and binary writer objects.

  observation_file_reader_bin_backward* reverse_binary_reader;
  observation_file_writer_bin*          binary_writer;
  ...
  //
  // Check the number of parameters. We need at least two (besides
  // the name of the program): one for the input binary file and
  // another one for the output binary file.
  //

  if (argc < 3)
  {
    cout << "usage: test_bin_to_reverse_bin input_bin_file_name output_bin_file_name" << endl;
    return 1;
  }
  ...
  // Instantiate the new binary backwards reader.

  reverse_binary_reader = new observation_file_reader_bin_backward();
  ...
  reverse_binary_reader->set_buffer_size(5 * 1024); // Just 5 Kbytes.

  // Try to open the file.

  io_status = reverse_binary_reader->open(infile_name.c_str());
  if (io_status != 0)
  {
    delete identifier;
    delete the_tags;
    ...
    cout << "[ERROR] open (input binary): " << io_status << endl;
    return 1;
  }

  // Instantiate the new binary writer.

  binary_writer = new observation_file_writer_bin();

  //
  // Set the maximum size of the writer's buffer in BYTES.
  // If not set, the writer will use a default buffer size,
  // that may be checked using query _default_buffer_size()
  //

  binary_writer->set_buffer_size(5 * 1024); // Just 5 Kbytes.

  // Try to open the file.

  io_status = binary_writer->open(outfile_name.c_str());
  if (io_status != 0)
  {
    delete identifier;
    delete the_tags;
    ...
    cout << "[ERROR] open (output binary): " << io_status << endl;
    return 1;
  }

  //
  // Try to read the whole contents of the binary file and, simultaneously,
  // write it to the binary output one.
  //
  ...
  while (true)
  {
    ...
    io_status = reverse_binary_reader->read_type(type);
    if (io_status != 0)
    {
      //
      ...
      if (reverse_binary_reader->is_eof())
      {
    ...
    io_status = reverse_binary_reader->read_active_flag(active);
    if (io_status != 0)
    {
    ...
    io_status = reverse_binary_reader->read_identifier(identifier, identifier_width);
    if (io_status != 0)
    {
    ...
    if (type == 'l')
    {
      io_status = reverse_binary_reader->read_instance_id(instance_identifier);
      if (io_status != 0)
      {
    ...
    io_status = reverse_binary_reader->read_time(time);
    if (io_status != 0)
    {
    ...
    if (type == 'l')
    {
      ...
      io_status = reverse_binary_reader->read_l_data(n_tags,              the_tags,
      ...
      io_status = binary_writer->write_l(active, identifier, instance_identifier, time,
                                         n_tags, the_tags,
                                         n_expectations, the_expectations,
                                         n_covariance_values, the_covariance_values);
    ...
    else // (type == 'o')
    {
      ...
      io_status = reverse_binary_reader->read_o_data(n_par_iids, the_par_iids,
      ...
      io_status = binary_writer->write_o(active, identifier, time,
  ...
  io_status = reverse_binary_reader->close();
  if (io_status != 0)
  {
    cout << "[ERROR] close (input binary): " << io_status << endl;
    i = 1;
  }

  // Try to close the output file.

  io_status = binary_writer->close();
  if (io_status != 0)
  {
    cout << "[ERROR] close (output binary): " << io_status << endl;
    i = 1;
  }

  delete reverse_binary_reader;
