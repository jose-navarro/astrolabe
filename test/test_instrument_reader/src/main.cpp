...
{
  {
    ...
    svalue = "Sample instruments (obs-e) file";
    lineage.name_set(svalue);

    // Set the author

    svalue = "Instrument reader tester program";
...
(int argc, char** argv)
{
  ...
  string            instrument_file_name;

  // Synthetic (nonsensical) lineage to add to the ASTROLABE header file.

  lineage_data      output_file_lineage;

  // An ASTROLABE universal instrument reader

  instrument_reader reader;

  //
  // An ASTROLABE universal instrument writer. Used to write somewhere
  ...
  // storing data.
  //
  // Then, we'll use an instrument writer to write an output text file.
  // For a detailed explanation on how to set up and write an ASTROLABE
  // header + instrument file, check example test_instrument_writer.
  //

  cout << "Please, type the name of the output ASTROLABE header file : ";
  cin >> output_filename;

  cout << "Now type the name of the file with actual instrument data : ";
  cin >> instrument_file_name;
  ...
  // Configure the writer using the information just obtained.

  io_status = writer.set_data_channel(output_filename.c_str(),
                                      instrument_file_name.c_str(),
  ...
  while (true)
  {
    ...
    // tags or expectations are there for each type of
    ...
    io_status = reader.read_l_data(n_tags, the_tags,
